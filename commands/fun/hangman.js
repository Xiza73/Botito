module.exports = {
    ...
    **Modo de juego 2:** Te incluyes entre las menciones y Botito escoge una palabra al azar para que todos jueguen.`,
    ...
    run: async (client, message, args) => {
      ...
      let turn = 0
      let mencion = message.mentions.users.map(x  =>
        x.id);
      ...
      const img = "https://res.cloudinary.com/dnbgxu47a/image/upload/v1612912935/ahorcado";
      ...
      if(mencion.includes(message.author.id)) 
      {
        ...
        palabra = json[getRandom(0, json.length - 1)].nombre
      }else{
        ...
        await  canal.awaitMessages(m  =>  m.author.id == message.author.id && m.content.replace(/[^A-Za-z0-9áéíóú\u00f1]/g,"").length,
        {max:  1, time:  20000, errors:["time"]}).then(collected  => {
        palabra = collected.first().content.replace(/[^A-Za-z0-9áéíóú\u00f1 ]/g,"")
      ...
      ahorcado.on("end", game => {
          ...
          if(game.winned){ //Si el juego ha terminado y se ha descubierto toda la frase
              s += "El juego ha finalizado! La palabra era: **"+game.palabra+"**\n"
              +"Descubierto por: **"+players[turn]+"**"
              + "```"+game.ascii.join(" ")+"```"
              vida = 7;
          }else{ //Si ha terminado pero no han descubierto la frase
              s += "Han perdido! La palabra era: **"+game.palabra+"**\n"
              +"Último error: **"+players[turn]+"**"
              +"```\n"+game.ascii.join(" ")+"```"
          }
          const emb = {
            color: 0x0099ff,
      ...
      const e = {
          color: 0x0099ff,
          title: `Ahorcado`,
          description: "```\n"+ahorcado.game.ascii.join(" ")+"```**Empieza "+ players[turn] +"**"
      }
      turn = change(turn, players.length-1, 1)
      message.channel.send({ embed: e });

      const  colector = message.channel.createMessageCollector(msg  =>  msg.author.id == ahorcado.game.turno && /[A-Za-z0-9áéíóú\u00f1]/.test(msg.content) && msg.content.length == 1);
      ...
      colector.on('collect', msg  => {
        ...
        if(!encontrado){
          if(ahorcado.game.ascii.includes(msg.content)){
            s += "- La letra \""+msg.content+"\" ya se encuentra en la palabra!";
          }else{
            s += "- Vaya! Parece que la letra "+msg.content+" no se encontraba en la palabra!";
          }
...
function change(turno, ultimo, i){
  turno += i
  if(turno < 0)
    turno = ultimo
  if(turno > ultimo)
