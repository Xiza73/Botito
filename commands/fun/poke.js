...
const fetchData = async (id, message, client) => {
    try{
        const fetch = require("node-fetch");
        const res = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`)
        const data = await res.json()
        pintarCard(modelData(data, id), message, client)
    }catch(error){
        console.log(error)
    }
}

const fetchType = async (id, message, client) => {
    try{
        const fetch = require("node-fetch");
...
function pintarCard(poke, message, client){
    let type = ""
